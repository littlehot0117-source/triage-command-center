...
function renderHistory() {
  ...
  if (history.length === 0) {
    ...
    historyList.innerHTML = sorted.map(h => {
      const dateStr = new Date(h.timestamp).toLocaleTimeString('zh-TW', { hour12: false });
      return `
        <div class="history-item ${h.color}">
          <div>
            <div style="font-weight:700;">患者 ${h.id}</div>
            <div style="font-size:11px; color:var(--text-muted); margin-top:2px;">${h.desc || '無描述'} | ${h.location}</div>
          </div>
          <div style="text-align:right;">
            <span class="triage-badge ${h.color}" style="font-size:10px; padding:3px 8px;">${getTriageLabel(h.color)}</span>
